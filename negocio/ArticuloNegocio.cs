...
namespace negocio
{
    public class ArticuloNegocio
    {
        ...
        public Articulo listar(int id)
        {
            ...
            try
            {
                datos.setearConsulta($"SELECT A.Id, A.Nombre, A.Codigo, A.Descripcion, M.Descripcion AS Marca, C.Descripcion AS Categoria, A.Precio, A.IdMarca, A.IdCategoria FROM ARTICULOS A LEFT JOIN MARCAS M ON M.ID = A.IdMarca LEFT JOIN CATEGORIAS C ON C.ID = A.IdCategoria WHERE A.Id = {id}");
                datos.ejecutarLectura();
                ...
                while (datos.Lector.Read())
                {
                    ...
                    decimal dosDecimales = (decimal)datos.Lector["Precio"];
                    articulo.Precio = Math.Truncate(dosDecimales * 100) / 100;
            ...
            return articulo; // Devuelve el artículo actualizado
        }
